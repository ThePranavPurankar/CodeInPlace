...
import sys
import string
import common_elements as ce

def line_to_terms(line_string):
    #input the line in a file, output a list of terms
    final_terms_list = []
    ...
    for i in range(len(raw_terms_list)):
        ...
        if term not in final_terms_list and len(term)>0:
...
def create_index(filenames, index, file_titles):
    ...
    #for each file open up the content
    #use file name and title to update the file_titles dict
    #loop through the content, strip off the punctuation and parse out the terms, update index dict
...
def search(index, query):
    ...
    []
    """
    query_list = query.split() #assuming query has no punctuation 
    result = []
    for i, term in enumerate (query_list):
        ...
        if i == 0:
            ...
            result = ce.common(result,temp_result)
...
if __name__ == '__main__':
    main()
    doctest.testmod()   #check all test using python searcheninge.py -v
